...
class HRI_AirSim(gym.Env):
    ...
    def reset(self, initial_x = None, initial_y = None):
        ...
        if self.mission == 'landing':
            ...
            if initial_x == None:
                initial_x = 1.5*(2*np.random.rand()-1)
            if initial_y == None:
                initial_y = 2.5*(2*np.random.rand()-1)

            # # overwrite random initialization
            # # test data
            # initial_x = 0.92456
            # initia_y = 0.83418
            # # train data
...
class HRI_AirSim_Landing(HRI_AirSim):
    ...
    def step(self, action):
        ...
        # check if too high
        #if pos_z < -12: #
        #    print('[*] Too high. Reseting...')
        #    done =1
        # check if close to land
        #if pos_z > 0:
            #print('[*] Landing...')
            #self.client.land(max_wait_seconds = 10)
        #    done = 1
        # max number of steps
        ...
        pad_z_loc = moving_vehicle_position.z_val
        dist_to_pad = np.sqrt(( (self.imu[0] - pad_x_loc)**2 + (self.imu[1] - pad_y_loc)**2))
        ...
        #         reward = 0.
        #     # display results
        #     print('[*] (distance to pad: {:6.4f} | threshold: {})\n'.format(
        #         dist_to_pad, dist_threshold))
        #
        # else:
        #     # reward just at the end of the task
        #     reward = 0.
    ...
    def _old_find_landing_pad(self, img):
        """ Find landing pad (circle) in camera image.

        Args:
            image: RGB or Grayscale image

        Return:
            x,y,r: x and y pixel coordinate, plus radius of pad
        """
        # check if image is grayscale
        # (needs to be grayscale to find circles using pixel gradient)
        if len(img.shape) != 2: # not in grayscale format
            img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

        # find circle, returns coordinates of largest one
        img = cv2.medianBlur(img,5)
        circles = cv2.HoughCircles(img,cv2.HOUGH_GRADIENT,1,20,
                            param1=50,param2=20,minRadius=0,maxRadius=0)

        if circles is not None:
            largest_circle = np.argmax(circles[0][:,2])
            circle = np.uint16(np.around(circles[0][largest_circle]))
